...
int main(int argc, char *argv[]) {
  ...
  for (Loop = 1; Loop < argc; Loop++) {
    ...
    printf("\t -o\tTakes the shortest available prefix an maximum runs\n");
  ...
  else if (ProgressType == PROGRESSNONE) {
  		if (Binary) printf("%.2f\t%.2f\t%.5f\n",Complexity,invlogint(Complexity)/log(2.0), invlogint(Complexity)/DataLen/log(2.0));
  		else printf("%.2f\t%.2f\t%.5f\n",Complexity,invlogint(Complexity), invlogint(Complexity)/DataLen);
  }
  Finalise();
  return 0;
}
